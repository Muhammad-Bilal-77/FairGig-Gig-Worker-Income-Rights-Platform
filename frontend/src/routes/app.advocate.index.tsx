...
function AdvocateOverview() {
  ...
  const MOCK_SUMMARY = {
    total_confirmed_shifts: 4520,
    total_workers_active_30d: 1240,
    platforms_tracked: ["Foodpanda", "Careem", "InDrive", "Bykea", "Yango"],
    highest_deduction_rate: { platform: "Foodpanda", city_zone: "Lahore Central", rate: 0.285 },
    lowest_deduction_rate: { platform: "InDrive", city_zone: "Karachi South", rate: 0.12 },
    most_complained_platform: "Foodpanda",
    vulnerability_flag_count: 42,
    avg_monthly_income: 22500,
    open_complaints_count: 156,
  };
    ...
    async function loadData() {
      try {
        ...
        const [
          ...
          api.analytics.getPlatformHealth().catch(() => null),
          api.analytics.getCorrelations().catch(() => null)
        ]);

        const finalSummary = summaryData || MOCK_SUMMARY;
        setSummary(finalSummary);

        // Generate AI Insight
        const platform = finalSummary.most_complained_platform || "Platform algorithms";
        let highestRate = 28.5;
        let highestPlatform = platform;

        if (finalSummary.highest_deduction_rate) {
          highestRate = (finalSummary.highest_deduction_rate.rate * 100);
          highestPlatform = finalSummary.highest_deduction_rate.platform;
        }
        ...
        if (topComplaintsData?.complaints?.length > 0) {
          ...
          focusCity = topComplaintsData.complaints[0].city_zone || "major zones";
        }

        const maxVuln = finalSummary.vulnerability_flag_count || 0;
        setAiInsight({
        ...
        if (healthData?.metrics) {
          setRadarData(healthData.metrics);
        }

        if (correlationData?.data) {
          setComposedData(correlationData.data);
        }

        if (statsData?.by_category) {
          setPieData(statsData.by_category.map((c: any) => ({
            name: c.category,
            value: c.count
          })));
        }
        ...
        setError(err?.message || "An unexpected error occurred while compiling system analytics.");
      } finally {
        setLoading(false);
      }
    }
    ...
    <PageContainer>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard
          ...
          value={summary ? summary.total_workers_active_30d.toLocaleString() : "..."}
          hint="Across Pakistan"
          icon={Users}
          delta={{ value: 8, positiveIsGood: true }}
        />
        <StatCard
          label="Avg monthly income"
          value={summary ? formatPKR(summary.avg_monthly_income) : "..."}
          hint="Per active worker"
          icon={Wallet}
        ...
        <StatCard
          ...
          value={summary ? `${summary.vulnerability_flag_count}` : "..."}
        ...
        <StatCard
          ...
          value={summary ? summary.open_complaints_count.toLocaleString() : "..."}
