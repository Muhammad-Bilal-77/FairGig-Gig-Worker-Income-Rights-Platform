...
async def setup_test_data():
    """Insert test shifts with unusual gap."""
    conn = await asyncpg.connect(DB_URL)
    
    try:
        # Create a test worker
        worker_id = uuid4()
        email = f"test-long-gap-{int(time.time())}@seed.com"
        ...
        # Insert 5 shifts with normal gaps (1-2 days apart)
        for i in range(5):
            shift_date = datetime.now().date() - timedelta(days=50 - i*2)
        ...
        # Insert a long gap (21 days, which is > 3x normal 2 day gap and > 7 days)
        last_shift_date = datetime.now().date() - timedelta(days=50 - 4*2)  # 42 days old
        anomaly_shift_date = last_shift_date - timedelta(days=21)  # 63 days old (21 day gap)
        ...
        await conn.execute(
            ...
            anomaly_shift_date, 5.0, gross_earned, platform_deduction,
            net_received, 'CONFIRMED'
        )
        
        print(f"\n✓ Created test worker: {email}")
        print(f"  Worker ID: {worker_id}")
        print(f"  - 5 recent shifts with 1-2 day gaps")
        print(f"  - 1 older shift with 21-day gap before it (> 3x normal 2-day gap and > 7 days)")
...
print("="*70)
print("  ANOMALY DETECTION TEST FOR LONG SHIFT GAP")
print("="*70)
...
if r.status_code == 200:
    ...
    if anomalies:
        ...
        print(f"\n   ✗ No anomalies detected (EXPECTED long gap NOT found)")
else:
